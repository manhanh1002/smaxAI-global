...
async function verifyModels() {
    ...
    try {
        ...
        const modelIds = (data as any).data.map((m: any) => m.id);
        console.log('Available models:', modelIds);

        // Test with a different model if available
        const testModel = modelIds.find((m: string) => m !== 'gpt-4o-mini' && (m.includes('gpt-4') || m.includes('gpt-3.5')));
        ...
        if (testModel) {
            ...
            if (!chatRes.ok) {
                 ...
                 console.log(`✅ Success with ${testModel}:`, (chatData as any).choices[0].message.content);
            }
        } else {
